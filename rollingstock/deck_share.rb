...
module Rollingstock
  ...
  def self.deck_share(placemat_only = false)
    Squib::Deck.new(
      ...
      background color: (0..CORP_NAMES.length * CORP_SHARES - 1).to_a.map { |n|
      ...
      png file: (0..CORP_NAMES.length * CORP_SHARES - 1).to_a.map { |n|
        "images/corplogos/#{CORP_NAMES[n / CORP_SHARES].downcase}.png"
      ...
      text str: ((0..CORP_SHARES - 1).to_a * CORP_NAMES.length).map { |n|
        if n.zero?
          '<i>President</i>'
        else
      ...
      rect x: 75 + SHARE_STRIPE_INSET, width: SHARE_STRIPE_WIDTH, height: 600,
           fill_color: (0..CORP_NAMES.length * CORP_SHARES - 1).to_a.map { |n| CORP_COLORS[n / CORP_SHARES] }
